...
if !exists('g:pear_tree_pairs') || get(b:, 'pear_tree_did_rust_ftplugin', 0)
    finish
endif
let b:pear_tree_did_rust_ftplugin = 1
...
let b:undo_ftplugin .= 'unlet! b:pear_tree_did_rust_ftplugin b:pear_tree_pairs'
...
" an opening angle bracket with zero or more following lifetime parameters, or
" if the character immediately before the cursor is an ampersand.
let s:patterns = ['\w', '<\s*\(''\s*[a-z]\s*,\s*\)*', '&']
